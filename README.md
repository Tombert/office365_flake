...
| Excel quits without a message on KDE Plasma under Wayland (SteamOS desktop mode), e.g. now and then when closing the Format pane of a chart; the debug log ends with `wp_cursor_shape_device_v1#…: error 1: unknown cursor shape` | GE's Wayland driver sends the four-way arrow cursor Office shows over a selected chart or shape (`OCR_SIZEALL`) as the cursor-shape-v1 shape `all_resize`, which only version 2 of the protocol has. KWin offers version 1 and ends the connection over it; sway and other wlroots compositors offer version 2, which is why it does not happen there. The patched `winewayland.so` sends `move` instead (version 1, the same arrows); see `wayland-fix/BUILD.md` |
