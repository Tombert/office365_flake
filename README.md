...
| `MS365_HOST_DRIVERS` | unset | on systems other than NixOS the launcher hands Proton the flake's Mesa (see the fixes table); `1` turns that off, e.g. when `/run/opengl-driver` is provided some other way |
...
| `MS365_NO_COMMAND_LISTS` | unset | `1` turns off the shim's Direct2D command-list replay (histograms and the other newer chart types, see the fixes table) |
...
| OneNote's notebook and page lists stay black and its page is black until the window is resized; Excel's cell grid is sometimes blank until a resize | GE's Wayland driver gives each child window with its own swapchain a subsurface below the window surface and stacks it there when it is created or moved, so the one configured last ends up on top whatever the Win32 z-order says: OneNote's full-size navigation background (the bottom sibling, created after the lists) covered them, Excel's `XLDESK` covered the grid. `wayland-fix/client-surface-zorder.patch` restacks all client subsurfaces of a window in Win32 order whenever one is stacked and whenever a child window's z-order changes (Excel shows its in-cell editor at the bottom of its siblings and raises it above the grid a moment later; without the second part typing into a cell showed nothing until Enter); `wayland-fix/winewayland.so` is GE-Proton11-7's driver built with it (`wayland-fix/build.sh`), and the flake swaps it into the runner. Worth sending upstream to GE |
| Excel quits without a message on KDE Plasma under Wayland (SteamOS desktop mode), e.g. now and then when closing the Format pane of a chart; the debug log ends with `wp_cursor_shape_device_v1#…: error 1: unknown cursor shape` | GE's Wayland driver sends the four-way arrow cursor Office shows over things that can be moved (`OCR_SIZEALL`) as the cursor-shape-v1 shape `all_resize`, new in version 2 of the protocol, when the compositor offers version 2. KWin 6.4 offers version 2 but still rejects shapes past version 1's last one and ends the connection over it (fixed in later KWin); sway and other wlroots compositors accept it. The patched `winewayland.so` sends `move` instead (version 1, the same arrows); see `wayland-fix/BUILD.md` |
| With Steam's on-screen keyboard (Steam Deck), `(` and `)` come out as `9` and `0`, and other shifted symbols unshifted; capital letters work | The on-screen keyboard types shifted symbols by setting Shift in the compositor's modifier state without a Shift key event. Linux apps translate keys with that state; Windows apps go by the key state, which only key events change, and GE's Wayland driver copies only the lock keys into it. The patched `winewayland.so` presses Shift (or Ctrl, Alt) for such a modifier before the key and releases it afterwards (`wayland-fix/virtual-modifiers.patch`) |
...
| On systems other than NixOS (SteamOS, Arch, Fedora, ...) Word exits at once with status 5; the terminal shows `radeonsi: driver missing` / `egl: failed to create dri2 screen` | nixpkgs' umu runs Proton in a NixOS-style sandbox whose own `/usr` has the GL/Vulkan loaders but no drivers; it finds drivers only through NixOS's `/run/opengl-driver`, so elsewhere there is no Vulkan for Office's Direct3D (DXVK). When `/run/opengl-driver` has no Vulkan drivers, the launcher puts the Mesa from the flake's nixpkgs on `XDG_DATA_DIRS` (Vulkan) and `__EGL_VENDOR_LIBRARY_FILENAMES` (EGL); pressure-vessel imports them like host drivers. Mesa only: proprietary NVIDIA drivers outside NixOS are not picked up |
...
| Histograms, Pareto, box and whisker, waterfall, treemap, sunburst and funnel charts show an empty frame | Office draws these charts into a Direct2D command list and then draws the list onto the sheet. Wine's d2d1 (11.16 too) fails `EndDraw` on a command-list target with `E_NOTIMPL` and `DrawImage` draws only bitmaps, so the recording is lost. The shim wraps the device-context vtable: `EndDraw` on a command list succeeds, and `DrawImage` of a command list replays it with `ID2D1CommandList::Stream` into a sink that forwards each command (transforms offset by the image position, clipped to the image rectangle) to the target context. `MS365_NO_COMMAND_LISTS=1` turns it off |
| Saving a workbook with one of those charts fails: "Errors were detected while saving"; Continue saves it without the chart | chart.dll writes the chart part's namespace declarations as `WriteAttributeString(L"xmlns", L"a", L"http://www.w3.org/2000/xmlns/", uri)`. Windows' xmllite takes the `xmlns` prefix together with its own namespace URI; Wine's returns `WR_E_XMLNSPREFIXDECLARATION` for any URI there, and chart.dll throws `IvyPersistence::PersistenceError`. The shim wraps `IXmlWriter::WriteAttributeString` on Wine's shared writer vtable and drops that URI, which writes the same `xmlns:a` declaration. Worth sending upstream to Wine |
| Under X11 (`MS365_WAYLAND=0`, or a desktop without Wayland) Excel crashes at start-up in combase (seen under Xwayland) | Under X11 Proton starts Xalia, its gamepad helper for game launchers, next to every app (`PROTON_USE_XALIA` defaults to 1 there). Xalia walks the focused window's accessibility tree from its own process, so Office's UI threads get a steady stream of cross-process COM calls (`IAccessible`, `IAccessible2`). When a thread uninitializes COM while such a call is queued, Wine's combase has already cleared the thread's apartment but still delivers the call to its message window, and `rpc_execute_call` dereferences the missing apartment; Windows fails the call with `RPC_E_DISCONNECTED`. The launcher sets `PROTON_USE_XALIA=0` (Xalia does nothing for Office; set it to 1 to keep it), and the shim subclasses each COM apartment window and fails calls that reach a thread without its apartment the way Windows does. Worth sending upstream to Wine |
