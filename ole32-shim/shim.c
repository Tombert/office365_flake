...
/* ---- Direct2D: rectangle CombineWithGeometry -------------------------------------------------
 * Wine's d2d1 has no boolean geometry operations (every CombineWithGeometry is a stub). Office cuts
 * small glyphs out of button rectangles with ID2D1RectangleGeometry::CombineWithGeometry(EXCLUDE):
 * the formula bar's cancel/enter/fx and expand buttons, the sheet-tab scroller. With nothing in the
 * sink those buttons come out as black squares. Rectangle minus shape is exact without a general
 * clipper: flatten the shape (Wine's own Simplify, LINES), clip each polygon to the rectangle
 * (Sutherland-Hodgman against its four edges), and emit the rectangle plus the clipped polygons
 * filled even-odd. XOR keeps the shape unclipped, INTERSECT is the clipped shape alone. UNION (not
 * seen from Office) and self-overlapping shapes stay approximate or E_NOTIMPL. */
typedef struct { float x, y; } my_point_f;
typedef struct { float left, top, right, bottom; } my_rect_f;
enum { MY_COMBINE_UNION, MY_COMBINE_INTERSECT, MY_COMBINE_XOR, MY_COMBINE_EXCLUDE };
#define D2D_FACTORY_CREATERECTGEOMETRY 5
#define D2D_GEOM_SIMPLIFY   9
#define D2D_GEOM_COMBINE    11
#define D2D_RECTGEOM_GETRECT 17
/* ID2D1SimplifiedGeometrySink slots */
#define SINK_SETFILLMODE 3
#define SINK_BEGINFIGURE 5
#define SINK_ADDLINES    6
#define SINK_ENDFIGURE   8
typedef HRESULT (WINAPI *pCombine)(void *, void *, int, const void *, float, void *);
static pCombine real_RectCombine;

/* proxy sink: forwards figures, swallows SetFillMode and Close */
struct proxy_sink { void *const *vtbl; void *real; };
static HRESULT WINAPI ps_QueryInterface(struct proxy_sink *p, REFIID iid, void **out) { (void)iid; if (!out) return E_POINTER; *out = p; return S_OK; }
static ULONG WINAPI ps_AddRef(struct proxy_sink *p) { (void)p; return 2; }
static ULONG WINAPI ps_Release(struct proxy_sink *p) { (void)p; return 1; }
static void WINAPI ps_SetFillMode(struct proxy_sink *p, int mode) { (void)p; (void)mode; }
static void WINAPI ps_SetSegmentFlags(struct proxy_sink *p, int f) { typedef void (WINAPI *fn)(void *, int); COM_CALL(p->real, 4, fn)(p->real, f); }
static void WINAPI ps_BeginFigure(struct proxy_sink *p, my_point_f pt, int begin) { typedef void (WINAPI *fn)(void *, my_point_f, int); COM_CALL(p->real, 5, fn)(p->real, pt, begin); }
static void WINAPI ps_AddLines(struct proxy_sink *p, const my_point_f *pts, UINT32 n) { typedef void (WINAPI *fn)(void *, const my_point_f *, UINT32); COM_CALL(p->real, 6, fn)(p->real, pts, n); }
static void WINAPI ps_AddBeziers(struct proxy_sink *p, const void *b, UINT32 n) { typedef void (WINAPI *fn)(void *, const void *, UINT32); COM_CALL(p->real, 7, fn)(p->real, b, n); }
static void WINAPI ps_EndFigure(struct proxy_sink *p, int end) { typedef void (WINAPI *fn)(void *, int); COM_CALL(p->real, 8, fn)(p->real, end); }
static HRESULT WINAPI ps_Close(struct proxy_sink *p) { (void)p; return S_OK; }
static void *const proxy_sink_vtbl[] = { (void *)ps_QueryInterface, (void *)ps_AddRef, (void *)ps_Release, (void *)ps_SetFillMode,
    (void *)ps_SetSegmentFlags, (void *)ps_BeginFigure, (void *)ps_AddLines, (void *)ps_AddBeziers, (void *)ps_EndFigure, (void *)ps_Close };

static void WINAPI bs_nop(void) {}
static HRESULT WINAPI bs_ok(void) { return S_OK; }
/* sink that collects flattened figures (Simplify with D2D1_GEOMETRY_SIMPLIFICATION_OPTION_LINES) */
struct poly_sink { void *const *vtbl; my_point_f *pts; UINT32 n, cap; UINT32 *starts; UINT32 nfig, figcap; BOOL oom; };
static void pl_push(struct poly_sink *s, my_point_f p)
{
    if (s->n == s->cap) {
        UINT32 cap = s->cap ? s->cap * 2 : 256;
        my_point_f *np = s->pts ? HeapReAlloc(GetProcessHeap(), 0, s->pts, cap * sizeof(*np)) : HeapAlloc(GetProcessHeap(), 0, cap * sizeof(*np));
        if (!np) { s->oom = TRUE; return; }
        s->pts = np; s->cap = cap;
    }
    s->pts[s->n++] = p;
}
static void WINAPI pl_BeginFigure(struct poly_sink *s, my_point_f pt, int begin)
{
    (void)begin;
    if (s->nfig == s->figcap) {
        UINT32 cap = s->figcap ? s->figcap * 2 : 16;
        UINT32 *nf = s->starts ? HeapReAlloc(GetProcessHeap(), 0, s->starts, cap * sizeof(*nf)) : HeapAlloc(GetProcessHeap(), 0, cap * sizeof(*nf));
        if (!nf) { s->oom = TRUE; return; }
        s->starts = nf; s->figcap = cap;
    }
    s->starts[s->nfig++] = s->n;
    pl_push(s, pt);
}
static void WINAPI pl_AddLines(struct poly_sink *s, const my_point_f *pts, UINT32 n) { for (UINT32 i = 0; i < n; i++) pl_push(s, pts[i]); }
static void WINAPI pl_AddBeziers(struct poly_sink *s, const my_point_f *pts, UINT32 n) { for (UINT32 i = 0; i < n; i++) pl_push(s, pts[3 * i + 2]); } /* not expected with LINES */
static void *const poly_sink_vtbl[] = { (void *)ps_QueryInterface, (void *)ps_AddRef, (void *)ps_Release, (void *)bs_nop,
    (void *)bs_nop, (void *)pl_BeginFigure, (void *)pl_AddLines, (void *)pl_AddBeziers, (void *)bs_nop, (void *)bs_ok };

/* Sutherland-Hodgman: clip polygon in[0..n) against one rectangle edge into out; returns count */
static UINT32 clip_edge(const my_point_f *in, UINT32 n, my_point_f *out, int edge, float v)
{
    UINT32 m = 0;
    for (UINT32 i = 0; i < n; i++) {
        my_point_f a = in[i], b = in[(i + 1) % n];
        float da, db;
        switch (edge) {
        case 0: da = a.x - v; db = b.x - v; break;      /* keep x >= left */
        case 1: da = v - a.x; db = v - b.x; break;      /* keep x <= right */
        case 2: da = a.y - v; db = b.y - v; break;      /* keep y >= top */
        default: da = v - a.y; db = v - b.y; break;     /* keep y <= bottom */
        }
        if (da >= 0) out[m++] = a;
        if ((da >= 0) != (db >= 0)) {
            float t = da / (da - db);
            my_point_f c = { a.x + t * (b.x - a.x), a.y + t * (b.y - a.y) };
            out[m++] = c;
        }
    }
    return m;
...
static LONG g_combine_logs;
static HRESULT WINAPI my_RectCombine(void *rect_geom, void *other, int mode, const void *transform, float tolerance, void *sink)
{
    typedef void (WINAPI *pGetRect)(void *, my_rect_f *);
    typedef HRESULT (WINAPI *pSimplify)(void *, int, const void *, float, void *);
    typedef void (WINAPI *pSetFillMode)(void *, int);
    my_rect_f r;
    HRESULT hr = real_RectCombine ? real_RectCombine(rect_geom, other, mode, transform, tolerance, sink) : E_NOTIMPL;
    if (hr != E_NOTIMPL || !other || !sink) return hr;   /* a Wine that can combine answers itself */
    COM_CALL(rect_geom, D2D_RECTGEOM_GETRECT, pGetRect)(rect_geom, &r);
    typedef void (WINAPI *pBegin)(void *, my_point_f, int);
    typedef void (WINAPI *pLines)(void *, const my_point_f *, UINT32);
    typedef void (WINAPI *pEnd)(void *, int);
    if (mode == MY_COMBINE_UNION) return hr;   /* not needed by Office so far */
    /* the second geometry as flattened polygons */
    struct poly_sink pl = { poly_sink_vtbl, NULL, 0, 0, NULL, 0, 0, FALSE };
    HRESULT sh = COM_CALL(other, D2D_GEOM_SIMPLIFY, pSimplify)(other, 1 /* LINES */, transform, tolerance > 0 ? tolerance : 0.25f, &pl);
    if (FAILED(sh) || pl.oom) {
        HeapFree(GetProcessHeap(), 0, pl.pts); HeapFree(GetProcessHeap(), 0, pl.starts);
        return hr;
    }
    COM_CALL(sink, SINK_SETFILLMODE, pSetFillMode)(sink, 0 /* D2D1_FILL_MODE_ALTERNATE */);
    if (mode != MY_COMBINE_INTERSECT) {
        my_point_f start = { r.left, r.top }, pts[3] = { { r.right, r.top }, { r.right, r.bottom }, { r.left, r.bottom } };
        COM_CALL(sink, SINK_BEGINFIGURE, pBegin)(sink, start, 0 /* D2D1_FIGURE_BEGIN_FILLED */);
        COM_CALL(sink, SINK_ADDLINES, pLines)(sink, pts, 3);
        COM_CALL(sink, SINK_ENDFIGURE, pEnd)(sink, 1 /* D2D1_FIGURE_END_CLOSED */);
    }
    /* XOR keeps the shape whole; EXCLUDE and INTERSECT only need its part inside the rectangle */
    my_point_f *t1 = HeapAlloc(GetProcessHeap(), 0, (pl.n * 2 + 8) * sizeof(my_point_f));
    my_point_f *t2 = HeapAlloc(GetProcessHeap(), 0, (pl.n * 2 + 8) * sizeof(my_point_f));
    for (UINT32 f = 0; t1 && t2 && f < pl.nfig; f++) {
        UINT32 from = pl.starts[f], to = f + 1 < pl.nfig ? pl.starts[f + 1] : pl.n, n = to - from;
        const my_point_f *poly = pl.pts + from;
        if (n < 3) continue;
        if (mode != MY_COMBINE_XOR) {
            n = clip_edge(poly, n, t1, 0, r.left);
            n = clip_edge(t1, n, t2, 1, r.right);
            n = clip_edge(t2, n, t1, 2, r.top);
            n = clip_edge(t1, n, t2, 3, r.bottom);
            poly = t2;
            if (n < 3) continue;
        }
        COM_CALL(sink, SINK_BEGINFIGURE, pBegin)(sink, poly[0], 0);
        COM_CALL(sink, SINK_ADDLINES, pLines)(sink, poly + 1, n - 1);
        COM_CALL(sink, SINK_ENDFIGURE, pEnd)(sink, 1);
    }
    HeapFree(GetProcessHeap(), 0, t1); HeapFree(GetProcessHeap(), 0, t2);
    HeapFree(GetProcessHeap(), 0, pl.pts); HeapFree(GetProcessHeap(), 0, pl.starts);
    if (InterlockedIncrement(&g_combine_logs) <= 10) OutputDebugStringA("ms365 ole32 shim: rectangle CombineWithGeometry answered (even-odd figures)");
    return S_OK;
}
...
static void patch_svg_vtable(void *factory)
{
    ...
    /* rectangle geometries share one vtable too */
    typedef HRESULT (WINAPI *pCreateRectGeom)(void *, const my_rect_f *, void **);
    my_rect_f unit = { 0, 0, 1, 1 }; void *geom = NULL;
    if (SUCCEEDED(COM_CALL(factory, D2D_FACTORY_CREATERECTGEOMETRY, pCreateRectGeom)(factory, &unit, &geom)) && geom) {
        void **slot = &(*(void ***)geom)[D2D_GEOM_COMBINE];
        DWORD old;
        if (*slot != (void *)my_RectCombine && VirtualProtect(slot, sizeof(void *), PAGE_READWRITE, &old)) {
            real_RectCombine = (pCombine)*slot;
            *slot = (void *)my_RectCombine;
            VirtualProtect(slot, sizeof(void *), old, &old);
            svg_log("ms365 ole32 shim: ID2D1RectangleGeometry::CombineWithGeometry wrapped");
        }
        COM_CALL(geom, 2, pUnkRelease)(geom);
    }
}
